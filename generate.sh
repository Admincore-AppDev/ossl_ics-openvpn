...
## Add "no-asm" to disable Perl-Assembler
...
[[ -n $ANDROID_NDK_ROOT ]] && ndk_toolchain_path=${ANDROID_NDK_ROOT}/toolchains/llvm/prebuilt/
...
target_compile_definitions(crypto PRIVATE $(echo $lib_cppflags))
...
function parse_c_args () {
  ## Parse Makefile to catch recent CPP/C-Flags for CMake
...
for patch in $(ls -1 ../patch/); do
	patch_info=$(cat ../patch/$patch | grep 'Subject:' | awk -F'Subject: ' '{print $2}' || true)
		if $(git apply -q ../patch/$patch); then
			echo -e "Applied Patch: \"$patch_info\"\n"
		else
			echo -e "Not applied Patch: \"$patch_info\"\n"
		fi
done
