...
target_link_options(crypto PRIVATE $(echo $lib_ldlags))
...
function gen_osslcnf () {
  if [[ -f crypto/buildinf-32.h && -f crypto/buildinf-64.h ]]; then
  ...
  if [[ -f include/crypto/bn_conf-32.h && -f include/crypto/bn_conf-64.h ]]; then
  ...
  if [[ -f include/openssl/configuration-32.h && -f include/openssl/configuration-64.h ]]; then
...
function make_in_files () {
  ...
  [ -f crypto/buildinf.h ] && mv -f crypto/buildinf.h crypto/buildinf-$bits.h
  [ -f include/crypto/bn_conf.h ] && mv -f include/crypto/bn_conf.h include/crypto/bn_conf-$bits.h
  [ -f include/openssl/configuration.h ] && mv -f include/openssl/configuration.h include/openssl/configuration-$bits.h
}

function parse_c_args () {
  ## Parse Makefile to get recent C/CPP/LD-Flags for CMake
...
if [ -n "$(git config user.name | grep "Admincore-AppDev")" ]; then
  read -p 'Expand Config_Args with Admincore defaults?: ' Adcore_ConfigArgs
  [ -n "$Adcore_ConfigArgs" ] && config_args+=$(cat ../Adcore_ConfigArgs)
fi
